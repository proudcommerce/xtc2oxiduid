...
class ImportHandler {
	...
	public function cleanUpBeforeImport() {
		...
		$oxDB = oxDb::getDb();//oxDb::FETCH_MODE_ASSOC); //true);
	...
	public function setLanguages() {
		...
		#oxRegistry::getConfig()->saveShopConfVar('aarr', 'aLanguages', serialize($aLanguages));
		#oxRegistry::getConfig()->saveShopConfVar('aarr', 'aLanguageParams', serialize($aConfLangs));
		oxRegistry::getConfig()->saveShopConfVar('aarr', 'aLanguages', $aLanguages);
		oxRegistry::getConfig()->saveShopConfVar('aarr', 'aLanguageParams', $aConfLangs);
	}
	...
	public function importCategories($shopSystem) {
		...
		$xtcReplaceFields = array();
		$xtcSelectFields = array();
		
		if ($shopSystem) {
			$xtcReplaceFields[0] = 'oxhidden,';
			$xtcSelectFields[0]	 = 'if (categories_status=1,0,1),';
		}
	 ...
	 * Customer importer
	 */
	public function importCustomers($shopSystem, $importPasswords) {
		...
		$xtcSelectFields[1] = 'c.customers_id+20000,';
		$xtcSelectFields[3] = 'now()';
		
		if ($shopSystem) {
			$sImported	= 'xt';
					
			$xtcReplaceFields[0] = 'oxrights,';
			$xtcReplaceFields[2] = 'oxustid,';
			
			$xtcSelectFields[0] = 'if (c.customers_status=0,\'malladmin\',\'user\'),';
			$xtcSelectFields[1] = 'if (c.customers_cid,c.customers_cid,c.customers_id+20000),';
			$xtcSelectFields[2] = 'c.customers_vat_id,';
			$xtcSelectFields[3] = 'if (c. customers_date_added,c. customers_date_added,now())';
		}
		...
		$sImportPasswords = '\'password\'';
		if ($importPasswords) {
			$sImportPasswords = 'c.customers_password';
		}
      ...
      REPLACE INTO oxuser (
		...
		$oRs = $oxDB->execute('select oxid,oxstreet,oxusername from oxuser where oxid<>"oxdefaultadmin" ORDER BY OXID');
		if ($oRs !== false && $oRs->recordCount() > 0) {
			$emails = array();
			while (!$oRs->EOF) {
	...
	public function importManufacturers() {
		...
		$aLangs = oxRegistry::getConfig()->getConfigParam('aLanguages');
	...
	public function importProducts($shopSystem = false) {
		...
		if ($shopSystem) {
			$xtcReplaceFields[0] = 'oxsearchkeys,';
			$xtcReplaceFields[1] = 'oxean,';
			
			$xtcSelectFields[0]	 = 'products_keywords,';
			$xtcSelectFields[1]  = 'products_ean,';
		}
        ...
        SELECT 
			...
			round(if (products_tax_class_id=1,products_price*(1+' . intval($this->_sXtcTaxRates[1]) . '),if (products_tax_class_id=2,products_price*(1+' . $this->_sXtcTaxRates[2] . '),products_price)),2),
			products_date_added,
			products_weight,
			products_name,
			products_url,
	...
	public function handleManufacturerImages() {
		...
		$image_dir		= oxRegistry::getConfig()->getMasterPictureDir() . 'manufacturer';

		$sQ = "SELECT oxid, oxicon FROM oxmanufacturers";
		$rs = oxDb::getDb(oxDb::FETCH_MODE_ASSOC)->execute($sQ); //true)->Execute($sQ);
		...
		while ($rs && $rs->recordCount() > 0 && !$rs->EOF) {
			...
			if (file_exists($sSrcName) && !is_dir($sSrcName)) {
				copy($sSrcName, $image_dir . "/icon/" . basename($sImg));
			}
			
			$sImg = basename($sImg);
			$sQ1 = "UPDATE oxmanufacturers SET oxicon = '$sImg' WHERE oxid = '" . $rs->fields["oxid"] . "'";
			oxDb::getDb(oxDb::FETCH_MODE_ASSOC)->execute($sQ1); //true)->Execute($sQ1);
			$rs->moveNext();
		}
	}
	...
	public function handleCategoryImages() {
		$sOscImageDir	= $this->_sOscImageDir;
		$sOcmDb			= $this->_sOcmDb;
		$image_dir		= oxRegistry::getConfig()->getMasterPictureDir() . 'category';
		...
		while ($rs && $rs->recordCount() > 0 && !$rs->EOF) {
			...
			if (file_exists($sSrcName) && !is_dir($sSrcName)) {
				copy($sSrcName, $image_dir . "/thumb/" . basename($sImg));
				copy($sSrcName, $image_dir . "/icon/" . basename($sImg));
			}

			$sImg = basename($sImg);
			$sQ1 = "UPDATE oxcategories SET oxthumb = '$sImg', oxicon='$sImg' WHERE oxid = '" . $rs->fields["oxid"] . "'";
			oxDb::getDb(oxDb::FETCH_MODE_ASSOC)->execute($sQ1); //true)->Execute($sQ1);
	...
	public function handleProductImages() {
		$sOscImageDir	= $this->_sOscImageDir;
		$sOcmDb			= $this->_sOcmDb;
		$image_dir		= oxRegistry::getConfig()->getMasterPictureDir() . 'product';
			...
			for ($i = 1; $i < $this->_iMaxImages; $i++) {
				...
				if ($sImg) {
					...
					if (file_exists($sSrcName) && !is_dir($sSrcName)) {
						...
						$sImg = basename($sImg);
						$sQ1 = "UPDATE oxarticles SET oxpic$i = '$sImg' WHERE oxid = '" . $rs->fields["oxid"] . "'";
						oxDb::getDb(true)->Execute($sQ1);
					}
				}
			}
