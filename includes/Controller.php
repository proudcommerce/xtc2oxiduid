...
class XTC2OXIDUI_Controller {
	...
	private function InitOxid($oxiddir) {
		...
		require_once( $oxiddir . 'bootstrap.php' );
	...
	private function MigrateCustomers($io, $shopSystem) {
		ob_start();
		$io->importCustomers($shopSystem, $this->GetSettingFromRequest('oxid-import-customers-passwords', true));
		return ob_get_clean();
	}
	...
	private function MigrateCategories($io, $shopSystem) {
		ob_start();
		$io->importCategories($shopSystem);
		$io->rebuildCategoryTree();
		return ob_get_clean();
	}

	private function MigrateProducts($io, $shopSystem) {
		ob_start();
		$io->importProducts($shopSystem);
